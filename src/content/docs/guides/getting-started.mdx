...
| macOS x64 / arm64 | `.dmg`, `.zip` | App currently uses the web streamer path; native streaming is gated off at runtime |
| Linux x64 / arm64 | `.AppImage`, `.deb` | App currently uses the web streamer path; `.deb` still declares common GStreamer dependencies for native backend work |
...
By default OpenNOW streams through Chromium/WebRTC in the renderer. The optional native streamer mode is experimental, currently Windows-only in the app, and uses a Rust child process with a GStreamer backend for video decode/rendering while Electron still owns session lifecycle and signaling. Native mode may have platform-specific bugs or missing media capabilities; if the platform, native executable, backend, protocol, or media capability is unavailable, OpenNOW reports the reason and falls back to the web streamer path. Report native-streamer issues on [GitHub Issues](https://github.com/OpenCloudGaming/OpenNOW/issues) or [Discord](https://discord.gg/8EJYaJcNfD).

On Linux, runtime native streaming is not currently exposed by the app. Developers experimenting with the backend should use system GStreamer libraries and plugins: install base/good/bad/ugly/libav plugins plus VAAPI or V4L2 packages for hardware decode where your distro provides them.

...
- Native streaming is accepted on Windows, macOS, and Linux, but platform media/runtime capabilities still decide whether a native session can start or must fall back to the renderer WebRTC path.

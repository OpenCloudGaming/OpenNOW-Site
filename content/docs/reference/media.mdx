...
- Recording bitrate is configurable in Settings. `Auto` leaves bitrate selection to `MediaRecorder`; custom values are rounded and clamped to **1–12 Mbps** before being passed as `videoBitsPerSecond`.
- Recording resolution (`720p` / `1080p` / `1440p`, default `720p`) and recording FPS (`30` / `60`, default `30`) are also configurable.
...
- Browser recording uses presented-frame capture with ordered chunk persistence and more reliable finalization on current desktop builds.

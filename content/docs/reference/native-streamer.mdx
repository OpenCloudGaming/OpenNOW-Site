...
import GStreamerRuntimeStrategyTable from '@/components/docs/GStreamerRuntimeStrategyTable';
...
The native streamer is an optional Rust child process used when `streamClientMode` is set to native mode on Windows. It is supervised by Electron and communicates over JSON-lines protocol version `2`.
...
<Callout type="info" title="Current platform gate">
The app currently treats native streaming as a Windows-only experimental feature. On macOS and Linux, `streamClientMode: "native"` is normalized back to `"web"` and streamer status reports the Windows-only unsupported message. The Rust/GStreamer codebase still carries macOS/Linux backend paths and release tooling may build native binaries there, but those platforms are not exposed as supported native-streaming targets yet.
</Callout>
...
| --- | --- | --- |
| `streamClientMode` | `"web"` | Default renderer WebRTC path; experimental native mode opts into the child process on Windows and is normalized back to web mode elsewhere |
...
Enabling native streaming from Settings opens an explicit experimental-feature confirmation before `streamClientMode` is changed to `"native"`. The setting only applies to new sessions. The regular `F3` stats HUD is an Electron overlay and is hidden while the external native renderer is active; `showNativeStreamerStats` controls the native renderer's own diagnostics overlay instead. While the native streamer window has focus, OpenNOW's Electron shortcuts and overlay actions do not fire; Alt+Tab back to the OpenNOW window to stop the stream, toggle overlays, or use other app shortcuts.
...
Linux package builds use distro GStreamer dependencies, but native streaming is still gated off in the app at runtime. Developers experimenting with the Rust backend on Linux should install GStreamer base/good/bad/ugly/libav plugins plus hardware packages such as VAAPI, V4L2, GL, Vulkan, and distro driver packages where available.
...
- The app is running on a non-Windows platform, where native mode is currently normalized back to web mode.

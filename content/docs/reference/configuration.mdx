...
The Settings About page can still send explicit feedback through the Feedback button. Feedback is a user-submitted event with a category and message; the user can optionally include app/system version details and recent redacted main + renderer logs, and the message is trimmed before capture.

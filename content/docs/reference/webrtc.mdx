...
Native mode emits protocol events instead: `status`, `input-ready`, `video-stall`, `video-transition`, `stats`, `log`, and `error`. See [Native Streamer](/docs/reference/native-streamer/) for experimental status, event fields, issue reporting, and fallback behavior.

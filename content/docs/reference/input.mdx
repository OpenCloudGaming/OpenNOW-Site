...
`allowEscapeToExitFullscreen` defaults to `false`. Escape is still used for pointer-lock release behavior; opting into Escape fullscreen exit makes fullscreen easier to leave but can conflict with in-game Escape usage. Intentional pointer-lock releases from OpenNOW controls, including the fullscreen shortcut, suppress the synthetic remote Escape packet so toggling fullscreen does not also open the game's Escape menu.
...
- When Chromium drops pointer lock because it consumed Escape before the renderer sees the key event, OpenNOW sends a synthetic Escape press to the stream and then tries to restore pointer lock. Intentional unlocks from OpenNOW UI/shortcuts skip that synthetic Escape.

...
`allowEscapeToExitFullscreen` defaults to `false`. Escape is still used for pointer-lock release behavior; opting into Escape fullscreen exit makes fullscreen easier to leave but can conflict with in-game Escape usage. When fullscreen or pointer lock is released by an intentional app shortcut such as `F8`, `F10`, or Stop Session, OpenNOW suppresses the one synthetic Escape that Chromium may otherwise generate from pointer-lock loss so the remote game does not receive an unintended menu/back action.
...
- If Chromium drops pointer lock because the user pressed Escape and the renderer did not receive an Escape keydown, OpenNOW sends a synthetic Escape keydown/keyup to the stream and then attempts to restore pointer lock; intentional app-driven pointer-lock/fullscreen exits opt out of that synthetic Escape.

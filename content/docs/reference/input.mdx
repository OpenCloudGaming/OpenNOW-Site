...
`allowEscapeToExitFullscreen` defaults to `false`. Escape is still used for pointer-lock release behavior; opting into Escape fullscreen exit makes fullscreen easier to leave but can conflict with in-game Escape usage. When pointer lock is released intentionally by OpenNOW controls such as the fullscreen toggle, pointer-lock toggle, or exit prompt, OpenNOW suppresses the fallback synthetic Escape so those controls do not also send Escape into the game.
...
- Browser-driven pointer-lock loss still sends a synthetic Escape to match GeForce NOW behavior, but intentional OpenNOW actions suppress that synthetic key event.

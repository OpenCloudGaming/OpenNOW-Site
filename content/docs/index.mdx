...
OpenNOW is an open-source GeForce NOW client with desktop, Android, iOS, and native [OpenNOW-Switch](/docs/guides/nintendo-switch) builds. These docs cover installation, streaming, settings, input, architecture, and troubleshooting.
...
A full rebuild with a controller-first UI and a new native streaming engine is on the way. See the [announcement post](https://x.com/Zortosdev/status/2093023076191440963) for a first look. These docs describe the current desktop stable line (**v0.5.5** and earlier).
</Callout>
...
<Cards>
  ...
  <Card title="Troubleshoot a problem" icon={<CircleHelp />} href="/docs/guides/troubleshooting" description="Fast checks for sign-in, launch, video, network, audio, input, and native-streamer issues." />
...
<Steps>
  <Step>
    **Download** the build for your device from the [OpenNOW download picker](/#downloads).
  </Step>
  <Step>
    **Install** the artifact for your platform: Windows `.exe`, macOS `.dmg`/`.zip`, Linux `.AppImage`/`.deb`, Android from Google Play or the beta `.apk`, Nintendo Switch `SwitchNOW.nro`, or the iOS TestFlight beta.
  </Step>
  <Step>
